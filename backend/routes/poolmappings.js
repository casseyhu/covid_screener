...
app.delete('/delete', (req, res) => {    
    ...
    POOLS_QUERY = POOLS_QUERY.slice(0, -1);
    const DELETE_POOLS_QUERY = `DELETE FROM Pool WHERE (poolBarcode) IN (${POOLS_QUERY})`;
    connection.query(DELETE_POOLS_QUERY, (err, result) => {
        if(err) {
            console.log(err)
            res.send(null)
        } else {
